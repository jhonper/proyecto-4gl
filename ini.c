...
static char *_module_name="ini.4gl";


A4GL_MAIN int main(int argc,char *argv[]);
#define fgldate long
static char const _rcsid[]="$FGLIdent: ini.4gl Compiler-1.61  Log:Not Set $";
...
A4GL_INTERNAL_FUNCTION void init_global_variables_ini_c5(void) {
...
static char *_functionName = "MAIN";
   ...
   A4GLSTK_pushFunction_v2(_functionName,_paramnames,_nargs,_module_name,6,_objData);

   A4GL_clr_ignore_error_list();
   /* CMD : E_CMD_IF_CMD Line 7 E_MET_MAIN_DEFINITION */
   ...
   if (A4GL_pop_bool()) {
      ...
      A4GL_clr_ignore_error_list();
      /* CMD : E_CMD_DISPLAY_CMD Line 9 E_MET_MAIN_DEFINITION */
      A4GLSTK_setCurrentLine(_module_name,9);
      aclfgli_clr_err_flg();
      A4GL_push_char("No es mayor");
      A4GL_push_int(-1);
      A4GL_push_int(-1);
      A4GL_display_at(1,0xffffffff);
         ERR_CHK_ERROR  { A4GL_chk_err(9,_module_name); }
   }
   A4GLSTK_popFunction_nl(0, 13);
   A4GL_fgl_end_4gl_0();
   return 0;
}
/* END OF MODULE */
