...
#define fgldate long
static char const _rcsid[]="$FGLIdent: main.4gl Compiler-1.61  Log:Not Set $";
...
static char *_functionName = "MAIN";
   ...
   A4GL_push_char("cliente");A4GL_open_form("cliente");
   ...
   while(_fld_dr!=0){
      if (_fld_dr== -100) {
         ...
         SET("s_screenio",&_sio_2,"nfields",A4GL_gen_field_chars((void ***)GETPTR("s_screenio",&_sio_2,"field_list"),(void *)GET("s_screenio",&_sio_2,"currform"),"nombre",0,"edad",0,NULL));
   ...
   aclfgli_clr_err_flg();
   A4GL_push_char("Nombre ingresado: ");

   A4GL_push_variable(&nombre,0x1e0000);
   A4GL_push_int(-1);
   A4GL_push_int(-1);
   A4GL_display_at(2,0xffffffff);
      ERR_CHK_ERROR  { A4GL_chk_err(11,_module_name); }
   A4GL_clr_ignore_error_list();
   /* CMD : E_CMD_DISPLAY_CMD Line 12 E_MET_MAIN_DEFINITION */
   A4GLSTK_setCurrentLine(_module_name,12);
   aclfgli_clr_err_flg();
   A4GL_push_char("Edad ingresada : ");
