...
static char *_module_name="ini3.4gl";


A4GL_MAIN int main(int argc,char *argv[]);
#define fgldate long
static char const _rcsid[]="$FGLIdent: ini3.4gl Compiler-1.61  Log:Not Set $";
...
static char *_functionName = "MAIN";
   ...
   /* CMD : E_CMD_PROMPT_CMD Line 4 E_MET_MAIN_DEFINITION */
   ...
   aclfgli_clr_err_flg();
   {
      ...
      A4GL_push_char("¿Cuál es tu nombre? ");
      int _dtype_hint=0;
      _dtype_hint=3276800;
      if (A4GL_start_prompt(&_sio_2,_attr_prompt,0,0,_attr_field,"",_style,_dtype_hint)) {
         ...
         A4GL_pop_var2(&nombre,0,50);
      }
   }
   END_BLOCK_1: ;
      ERR_CHK_ERROR  { A4GL_chk_err(4,_module_name); }
   A4GL_clr_ignore_error_list();
   /* CMD : E_CMD_DISPLAY_CMD Line 6 E_MET_MAIN_DEFINITION */
   A4GLSTK_setCurrentLine(_module_name,6);
   aclfgli_clr_err_flg();
   A4GL_push_char("Hola,  ");

   A4GL_push_variable(&nombre,0x320000);
   A4GL_push_char("Bienvenido");
   A4GL_push_int(-1);
   A4GL_push_int(-1);
   A4GL_display_at(3,0xffffffff);
